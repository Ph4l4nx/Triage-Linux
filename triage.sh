...
grep -v -e '^#' -e '^[[:space:]]*$' /etc/ssh/ssh_config >> "$output_file" 2>/dev/null
print_yellow "Configuracion de inmutabilidad en ficheros importantes:" >> "$output_file" 
{ for file in /etc/shadow /etc/passwd /etc/sudoers /etc/ssh/sshd_config; do attr=$(lsattr "$file" 2>/dev/null | awk '{print $1}'); echo -n "$file: "; if [[ $attr == *i* ]]; then echo "Inmutable ($attr)"; else echo "Modificable ($attr)"; fi; done; echo -e "\n\n";} >> "$output_file" 2>/dev/null
...
print_red "####                                       ####     " >> "$output_file"
print_red "###############################################     " >> "$output_file"
print_yellow "Volumnes montados:" >> "$output_file" 
mount  >> "$output_file" 2>/dev/null >> "$output_file"
print_yellow "USBs:" >> "$output_file" 
...
if [ -f /etc/os-release ]; then
    ...
    grep -vE '^\s*($|#)' /etc/apt/sources.list | while read -r line; do
        if es_repo_normal "$line"; then
            echo "Parece bueno: $line" >> "$output_file"
        else
